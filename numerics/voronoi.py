...
def build_velocity_data():
    ...
    for j in range(ITERS + 1):
        ...
        if GAMMA != 0:
            ...
            u = (1 - GAMMA) * u + GAMMA * u_pot
            v = (1 - GAMMA) * v + GAMMA * v_pot
...
def make_wrappers(x, y, u_arr, v_arr):
    ...
    def time_interp(getter, t, xp, yp):
        ...
        a = s - below
        return ((1 - a) * getter(below)((yp, xp)) + a * getter(above)((yp, xp))) * SCALE_FACTOR
...
def periodic_voronoi(points, x_range, y_range):
    ...
    for idx in central_indices:
        ...
        # Only keep polygons mostly inside plotting box
        cx = np.mean(vertices[:, 0])
        cy = np.mean(vertices[:, 1])

        if not (xmin - Lx <= cx <= xmax + Lx and ymin - Ly <= cy <= ymax + Ly):
            continue
...
def plot_area_pdf(areas):
    areas = areas[np.isfinite(areas)]
    areas = areas[areas > 0]

    if len(areas) == 0:
        print("No valid Voronoi areas to plot.")
        return
    ...
    if len(normalised_areas) > 2 and np.std(normalised_areas) > 0:
        ...
        xs = np.linspace(normalised_areas.min(), normalised_areas.max(), 300)
...
def animate_voronoi(history, x_range, y_range, wrap_positions):
    ...
    def update(frame):
        ...
        title.set_text(
            f"Voronoi tessellation, t = {time_hist[frame]:.2f} days"
        )
...
def main():
    ...
    x, y, u_arr, v_arr = build_velocity_data()
    ...
    print("Number of particles:", len(final_points))
    ...
    if len(final_areas) > 0:
        ...
        print("No valid Voronoi cells found.")

    plot_area_pdf(final_areas)
